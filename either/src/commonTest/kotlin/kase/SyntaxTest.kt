...
class SyntaxTest {
    ...
    @Test
    fun should_have_a_good_syntax() = when (val o: Either<Int, String> = Failure(69)) {
        is Failure -> {
            val f: Int = o.value
            expect(f).toBe(69)
        }
    ...
    fun should_be_able_to_map_a_failure() {
        val o: Either<Int, String> = Failure(69)
        val res = o.map {
            it.length
        }.map {
            it * 3
    ...
    fun should_not_crash_when_underlying_types_failure_types_change() {
        val o: Either<Exception, String> = Success("Hello")
    ...
    fun should_be_able_to_flat_map_successes() {
        val o: Either<Throwable, String> = Success("69")
    ...
    fun should_be_able_to_flat_map_errors() {
        val o: Either<Throwable, String> = Failure(IllegalStateException("Bad Number"))
        val res = o.flatMap { Success(it.toInt()) as Either<Throwable, Int> }.error()
